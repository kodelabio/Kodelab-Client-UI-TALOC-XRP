...
export const collateralTypesPrices = {
  generate: (types) => ({
    dependencies: () =>
      types.map((collateralTypeName) => [COLLATERAL_TYPE_PRICE, collateralTypeName]),
    computed: (...prices) => {
      return prices
    },
  }),
...
export const vaultCollateralAndDebt = {
  generate: (vaultType, vaultAddress) => ({
    ...
    computed: (encumberedCollateral, encumberedDebt) => [encumberedCollateral, encumberedDebt],
  }),
}
...
export const debtValue = {
  generate: (id) => ({
    dependencies: [
      ...
      [ANNUAL_STABILITY_FEE_ORIGINAL, [VAULT_TYPE, id]],
      [DATE_STABILITY_FEES_LAST_LEVIED, [VAULT_TYPE, id]],
    ],
    computed: (encumberedDebt, debtScalingFactor, duty, rho) => {
      try {
        const locked = encumberedDebt.times(debtScalingFactor)
        const secondsSinceDrip = Math.ceil((Date.now() - rho.getTime()) / 1000)
        
        return DAI(locked.times(duty.pow(secondsSinceDrip)))
      } catch (error) {
...
export const collateralTypesData = {
  generate: (types) => ({
    dependencies: () =>
      types.map((collateralTypeName) => [COLLATERAL_TYPE_DATA, collateralTypeName]),
...
export const userVaultsList = {
  generate: (address) => ({
    dependencies: ({ get }) => {
      const cdpManagerAddress = get('smartContract').getContractAddress('CDP_MANAGER')
      return [
...
export const collateralTypeCollateralization = {
  generate: (collateralTypeName, percentage = true) => ({
    ...
    computed: (debt, price, amount) => {
      ...
      const ratio = calcCollateralizationRatio(collateral, debt.toBigNumber()).times(100)
      return percentage ? ratio : { collateralValue: collateral, debtValue: debt }
    },
  }),
}
...
export const systemCollateralization = {
  generate: (vaultTypes) => ({
    ...
    computed: (...collateralizationValues) => {
      const { totalCollateralValue, totalDebtValue } = collateralizationValues.reduce(
