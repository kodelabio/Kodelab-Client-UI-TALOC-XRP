...
const decodeVatFrob = (data) => {
  const sig = ethAbi
    .encodeFunctionSignature('frob(bytes32,address,address,address,int256,int256)')
    .slice(2)
  const decoded = ethAbi.decodeParameters(
    [
      'bytes32', // ilk
...
export default async function getEventHistory(cdpManager, managedCdp, cache) {
  ...
  const OLD_MCD_CAT = cdpManager.get('smartContract').getContractAddress('OLD_MCD_CAT')
  ...
  const { NewCdp } = cdpManager.get('smartContract').getContract('CDP_MANAGER').interface.events

  const { Bite } = cdpManager.get('smartContract').getContract('MCD_CAT').interface.events
      ...
      return r.reduce(async (acc, { blockNumber: block, data, topics }) => {
        ...
        if (dart.lt(0) || dart.gt(0)) {
          ...
          const filteredJoinDaiEvents = joinDaiEvents.filter((daiEvent) =>
            cdpMoveEvents.some(
              (moveEvent) => moveEvent.transactionHash === daiEvent.transactionHash,
            ),
          )
          acc.push(
  ...
  const catBite = (address, fromBlock, toBlock) => ({
    ...
    result: (r) =>
      r.map((tx) => {
        const { topics, data, blockNumber: block, transactionHash: txHash } = tx
  ...
  const lookups = [
    {
      ...
      result: async (r) =>
        r.reduce(async (acc, { blockNumber: block, transactionHash: txHash }) => {
          acc = await acc
